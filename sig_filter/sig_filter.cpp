...
void help(){
  cout << "sig_filter -- read and process .sig datafiles\n"
          ...
          "       -F <value>  -- low frequency limit\n"
          "       -G <value>  -- high frequency limit\n"
          "       -t          -- use time-based length of fitting window (set with -w option) instead of points-based\n"
          "       -w <value>  -- window length (points, default 1024, or time, s); depends on -t filter option\n"
          "       -s <value>  -- window step (points or time, s; default equals to window); depends on -t filter option\n"
